...
req_cal_get = urllib.request.Request(
    'http://127.0.0.1:8000/api/v1/calendar/events', headers={"Authorization": f"Bearer {token}"}
)
try:
    print("Success:", urllib.request.urlopen(req_cal_get).read().decode())
