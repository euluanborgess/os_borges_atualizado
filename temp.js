        ...
        function renderNotifications() {
            ...
            if (notifications.length > 0) {
                ...
                notifications.forEach((n, idx) => {
                    html += `
                        <div class="p-3 hover:bg-[#1c1c28] transition-colors cursor-pointer border-b border-[#1e1e2e]" 
                             onclick="handleNotifClick('${n.lead_id}', ${idx})">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-500 text-xs shrink-0">
                                    <i class="fa-solid ${n.type === 'message' ? 'fa-comment' : 'fa-user-plus'}"></i>
                                </div>
                                <div class="flex-1">
                                    <p class="text-[11px] font-bold text-gray-200 line-clamp-1">${n.title}</p>
                                    <p class="text-[10px] text-[#5a5a6e] line-clamp-2">${n.content}</p>
                                </div>
                            </div>
                        </div>`;
                });
                ...
                list.innerHTML = '<div class="p-8 text-center text-[11px] text-[#5a5a6e]">Nenhuma notificação nova</div>';
            }
        }
        ...
        function toggleSidebar() {
            ...
            const spans = sidebar.querySelectorAll('nav button span, div p');
            const logoText = sidebar.querySelector('.font-bold.text-lg');
            
            const isCollapsed = sidebar.classList.toggle('w-[80px]');
            sidebar.classList.toggle('w-[250px]', !isCollapsed);
            
            icon.classList.toggle('fa-chevron-right', isCollapsed);
            icon.classList.toggle('fa-chevron-left', !isCollapsed);
            
            spans.forEach(s => s.classList.toggle('hidden', isCollapsed));
            if(logoText) logoText.classList.toggle('hidden', isCollapsed);
            
            localStorage.setItem('borges_sidebar_collapsed', isCollapsed);
        }

        function initSidebarState() {
            const collapsed = localStorage.getItem('borges_sidebar_collapsed') === 'true';
            if (collapsed) {
                const sidebar = document.getElementById('app-sidebar');
                const icon = document.getElementById('sidebar-toggle-icon');
                const spans = sidebar.querySelectorAll('nav button span, div p');
        ...
        let inboxTabFilter = 'queue'; // queue | me

        document.addEventListener('DOMContentLoaded', async () => {
            if (typeof initSidebarState === 'function') initSidebarState();
            // [KILL SWITCH] - Force Logout via URL
        ...
        function handleLogout() {
            console.log("Iniciando logout bruto...");
            try {
                localStorage.clear();
        ...
        function handleInitialRoute() {
            ...
            // Primeiro, checar se estamos em uma sub-página de Super Admin
            if (path.includes('super-tenant-detail')) {
                viewId = 'super-tenant-detail';
            } else if (path.includes('super')) {
                viewId = 'super-admin';
            }
            else if (path.includes('pipeline') || path.includes('leads')) viewId = 'pipeline';
            ...
            // Forçar carregamento correto da view
            if (viewId === 'super-tenant-detail') {
                // Tenta recuperar o ID do tenant da URL se houver, ou volta pro super
                const urlParams = new URLSearchParams(window.location.search);
                const tId = urlParams.get('id');
                if (tId) {
                    openTenantDetails(tId);
                } else {
                    switchView('super-admin', null, false);
                }
            } else {
        ...
        async function initializeApp() {
            ...
            // [FIX] - Clear Search Bar with multiple attempts to defeat browser autofill
            function clearSearchBar() {
                const s = document.getElementById('global-search-input');
                if (s) { s.value = ''; s.setAttribute('value', ''); }
            }
            clearSearchBar();
            requestAnimationFrame(clearSearchBar);
            setTimeout(clearSearchBar, 100);
            setTimeout(clearSearchBar, 500);
            ...
            // [ONBOARDING] - Show welcome modal on first login
            if (user.first_login) {
                setTimeout(() => {
                    if (typeof initWelcomeModal === 'function') {
                        initWelcomeModal(user.full_name);
                    }
                }, 500);
            }

            // [FIX] - Update Greeting Name
            const elGreetingName = document.getElementById('dash-greeting-name');
            if (elGreetingName) elGreetingName.innerText = user.full_name.split(' ')[0];
        ...
        async function fetchDashboardMock() {
            try {
                ...
                if (metrics.status === 'success') {
                    ...
                    if (elTotalLeads) animateValue("metric-total-leads", parseInt(elTotalLeads.innerText) || 0, data.total_leads || 0, 1000);
                    if (elMeetings) animateValue("metric-meetings", parseInt(elMeetings.innerText) || 0, data.total_events || 0, 1000);
                    if (elInteractions) animateValue("metric-interactions", parseInt(elInteractions.innerText) || 0, data.leads_waiting_human || 0, 1000);
                    ...
                    if (elStatTotal) animateValue("stat-total-leads", parseInt(elStatTotal.innerText) || 0, data.total_leads, 1000);
                    if (elStatHot) animateValue("stat-hot-leads", parseInt(elStatHot.innerText) || 0, data.hot_leads, 1000);
                    
                    if (elStatEstRev) elStatEstRev.innerText = `R$ ${data.estimated_revenue.toLocaleString('pt-BR')}`;
                    if (elStatClosedRev) elStatClosedRev.innerText = `R$ ${data.total_revenue.toLocaleString('pt-BR')}`;
                    
                    const convRate = data.total_leads > 0 ? ((data.total_events / data.total_leads) * 100).toFixed(1) : 0;
                    ...
                    if (elStatPendingTasks) animateValue("stat-pending-tasks", parseInt(elStatPendingTasks.innerText) || 0, data.pending_activities, 1000);
                    if (elStatActiveConvs) animateValue("stat-active-convs", parseInt(elStatActiveConvs.innerText) || 0, data.total_leads - data.leads_waiting_human, 1000);
                }
        ...
        function buildCard(lead) {
            ...
            // Encapsulando strings JSON para evitar erros de sintaxe no HTML
            const leadStr = btoa(unescape(encodeURIComponent(JSON.stringify(lead))));
            ...
            return `
                <div class="lead-card p-4 rounded-xl cursor-pointer transition-all duration-200 group ${hoverBorder}" 
                    ...
                    onclick='openLeadModalEncoded("${leadStr}")' 
        ...
        function renderInboxList() {
            ...
            const currentUser = JSON.parse(localStorage.getItem('borges_user') || '{}');

            // Filtrar por aba (Aguardando vs Meus)
            let filtered = allInboxLeads;
            if (inboxTabFilter === 'queue') {
                filtered = filtered.filter(l => !l.assigned_user_id);
            } else if (inboxTabFilter === 'me') {
                filtered = filtered.filter(l => l.assigned_user_id === currentUser.id);
            }
        ...
        function switchTasksTab(tab) {
            currentTaskTab = tab;
            const btnMe = document.getElementById('task-tab-me');
            const btnTeam = document.getElementById('task-tab-team');
            if(btnMe) btnMe.className = tab === 'me' ? "px-4 py-2 rounded-lg text-sm font-semibold transition-all border bg-cyan-500/15 text-cyan-300 border-cyan-500/30" : "px-4 py-2 rounded-lg text-sm font-semibold transition-all border bg-transparent text-[#5a5a6e] border-transparent hover:text-white";
            if(btnTeam) btnTeam.className = tab === 'team' ? "px-4 py-2 rounded-lg text-sm font-semibold transition-all border bg-cyan-500/15 text-cyan-300 border-cyan-500/30" : "px-4 py-2 rounded-lg text-sm font-semibold transition-all border bg-transparent text-[#5a5a6e] border-transparent hover:text-white";
        ...
        function openAddLeadModal() {
            const m = document.getElementById('modal-add-lead');
            if (m) {
        ...
        async function submitAddLead(e) {
            ...
            const payload = {
                ...
                temperature: document.getElementById('add-lead-temp').value
            };

            try {
                const res = await fetchWithAuth(`/api/v1/inbox/leads`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                if (res.ok) {
                    closeAddLeadModal();
                    fetchLeads(); // Recarregar a lista do Kanban
                } else {
        ...
        async function openInboxMaster(lead) {
            ...
            // [MULTI-AGENT UI LOCK]
            const currentUser = JSON.parse(localStorage.getItem('borges_user') || '{}');
            const isAssignedToMe = lead.assigned_user_id === currentUser.id;
            const isUnassigned = !lead.assigned_user_id;
            const isAdmin = currentUser.role === 'super_admin';

            const btnAccept = document.getElementById('btn-master-accept');
            const btnTransfer = document.getElementById('btn-master-transfer');
            const msgInput = document.getElementById('msg-input-master');
            const btnSend = document.getElementById('btn-send-master');
            const footer = document.getElementById('chat-master-footer');
            ...
            if (isUnassigned) {
                ...
                btnAccept.innerHTML = '<i class="fa-solid fa-hand-holding-hand mr-2"></i> ACEITAR ATENDIMENTO';
                btnTransfer.classList.add('hidden');
                msgInput.disabled = true;
                msgInput.placeholder = "Aceite o atendimento para digitar...";
                btnSend.disabled = true;
                if(btnHandoff) btnHandoff.classList.add('hidden');
                ...
                // Show Handoff buttons if assigned to me
                if(statusBadge) statusBadge.classList.remove('hidden');
                const isPaused = lead.is_paused_for_human === 1;
                if (isPaused) {
                    if(btnHandoff) btnHandoff.classList.add('hidden');
                    ...
                    if(btnHandoff) btnHandoff.classList.remove('hidden');
                    if(btnResume) btnResume.classList.add('hidden');
                    if(statusBadge) {
                ...
                // Atribuído a outro e eu não sou admin
                btnAccept.classList.add('hidden');
                btnTransfer.classList.add('hidden');
                msgInput.disabled = true;
                msgInput.placeholder = "Em atendimento por outro agente...";
                btnSend.disabled = true;
                if(btnHandoff) btnHandoff.classList.add('hidden');
                if(btnResume) btnResume.classList.add('hidden');
                if(statusBadge) {
                    statusBadge.classList.remove('hidden');
                    statusBadge.innerHTML = '<i class="fa-solid fa-lock mr-1.5"></i> Com outro Agente';
                    statusBadge.className = 'flex items-center bg-rose-500/10 text-rose-500 border border-rose-500/30 px-3 py-1.5 rounded-full text-[11px] font-bold';
                }
            }
            ...
            // [MULTI-AGENT] - Update Channel Badge
            const elChannelBadge = document.querySelector('#chat-master-details div.flex.items-center.gap-2.pl-2');
            if (elChannelBadge) {
                const isInsta = lead.channel === 'instagram';
                elChannelBadge.innerHTML = `
                    <span class="text-xs text-[#5a5a6e]">Canal:</span>
                    <span class="bg-${isInsta ? 'pink' : 'emerald'}-500/10 border border-${isInsta ? 'pink' : 'emerald'}-500/20 text-${isInsta ? 'pink' : 'emerald'}-500 text-[10px] uppercase font-bold px-2 py-1 rounded flex items-center gap-1.5">
                        <i class="fa-brands fa-${isInsta ? 'instagram' : 'whatsapp'} text-xs"></i> ${isInsta ? 'Instagram' : 'WhatsApp'}
                    </span>
                `;
            }
            ...
            // [MULTI-AGENT] - Populate detail fields
            const elAgent = document.getElementById('detail-master-agent');
            if (elAgent) {
                elAgent.innerText = lead.assigned_user_id ? (lead.assigned_user_name || "Agente Ativo") : "Fila de Espera (Ninguém)";
            }
            const elNotes = document.getElementById('detail-master-notes');
            if (elNotes) {
                elNotes.value = lead.internal_notes || "";
            }
            const elLastMsg = document.getElementById('detail-master-lastmsg');
            if (elLastMsg) {
                elLastMsg.innerText = timeAgoBr(lead.last_message_at || lead.updated_at);
            }

            // [ENTERPRISE] - Update Financials
            const elLtv = document.getElementById('detail-master-ltv');
            if (elLtv) {
                const val = (lead.ltv || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
                elLtv.innerText = val;
            }
            const elLastOrder = document.getElementById('detail-master-lastorder');
            if (elLastOrder) {
                if (lead.last_order_date) {
                    const d = new Date(lead.last_order_date);
                    elLastOrder.innerText = `${d.toLocaleDateString('pt-BR')} (R$ ${lead.last_order_amount.toFixed(2)})`;
                } else {
                    elLastOrder.innerText = "Sem compras";
                }
            }
            // [ENTERPRISE] - Update Tags & Stage
            const elTags = document.getElementById('detail-master-tags');
            if (elTags) {
                if (lead.tags && lead.tags.length > 0) {
                    elTags.innerHTML = lead.tags.map(t => `<span class="bg-[#1e1e2e] text-[#8b8b9e] text-[9px] px-1.5 py-0.5 rounded border border-[#3a3a4e]">${t}</span>`).join('');
                } else {
                    elTags.innerHTML = '<span class="text-[9px] text-[#3a3a4e]">Nenhuma tag</span>';
                }
            }
            const elStage = document.getElementById('detail-master-stage');
            if (elStage) {
                elStage.innerText = lead.pipeline_stage ? lead.pipeline_stage.toUpperCase() : "NOVO";
            }

            // [ENTERPRISE] - Update Timeline
            const elTimeline = document.getElementById('detail-master-timeline');
            if (elTimeline) {
                fetchWithAuth(`/api/v1/ws/inbox/leads/${lead.id}/timeline`)
                    .then(res => res.json())
                    .then(payload => {
                        const events = payload.data || [];
                        if (events.length > 0) {
                            elTimeline.innerHTML = events.map(e => `
                                <div class="flex items-start gap-2 text-[9px] border-b border-[#1e1e2e] pb-1.5 last:border-0 mb-1.5">
                                    <div class="w-1.5 h-1.5 rounded-full mt-1 shrink-0 ${e.action === 'assigned' ? 'bg-cyan-500' : 'bg-[#3a3a4e]'}"></div>
                                    <div class="flex-1">
                                        <p class="text-gray-300 font-bold leading-tight capitalize">${e.action === 'assigned' ? 'Assumido por' : e.action} ${e.user_name}</p>
                                        <p class="text-[8px] text-[#5a5a6e] font-mono">${new Date(e.created_at).toLocaleString('pt-BR')}</p>
                                    </div>
                                </div>`).join('');
                        } else {
                            elTimeline.innerHTML = '<span class="text-[9px] text-[#3a3a4e]">Conversa nova no sistema</span>';
                        }
                    }).catch(() => {});
            }
            ...
            if (mediaType === 'image' || mediaType === 'sticker') {
                ...
                return `
                    <div class="my-1.5">
                        ...
                        ${text && text !== '📷 Imagem' ? `<div class="mt-1.5 text-[12px] opacity-80">${text.replace(/\\n/g, '<br>')}</div>` : ''}
                    </div>`;
            }
                ...
                try {
                    ...
                    if (data.type === 'dashboard_update') {
                        ...
                        if (activeLeadId == data.lead_id) {
                            ...
                            // [ENTERPRISE] - Notificação Push p/ mensagens não lidas de outros leads
                            if (data.message && data.message.sender_type !== 'system') {
                                const lead = allInboxLeads.find(l => l.id === data.lead_id);
                                const name = lead ? (lead.name || lead.phone) : 'Novo Lead';
                                addNotification({
                                    type: 'message',
                                    title: name,
                                    content: data.message.content || '📎 Mídia recebida',
                                    lead_id: data.lead_id
                                });
                            }
                        }
                    } else if (data.type === 'lead_assigned') {
                        // [MULTI-AGENT] - Notificação de atribuição
                        const currentUser = JSON.parse(localStorage.getItem('borges_user') || '{}');
                        if (data.assigned_user_id === currentUser.id) {
                            addNotification({
                                type: 'assignment',
                                title: 'Lead Atribuído!',
                                content: `Você agora é o responsável pelo atendimento.`,
                                lead_id: data.lead_id
                            });
                        }
                        fetchInboxLeads();
        ...
        // Appointments data (fetched from API)
        let calendarAppointments = [];

        async function fetchAppointments() {
            try {
                const res = await fetchWithAuth('/api/v1/calendar/events');
                if (res.ok) {
                    const payload = await res.json();
                    calendarAppointments = (payload.events || []).map(e => ({
                        id: e.id,
                        title: e.title,
                        type: 'qualificacao',
                        ...
                        contact_name: (e.lead && e.lead.name) || e.title
                    }));
                }
            } catch (e) { console.error('fetchAppointments error:', e); }
            initCalendar();
        }
        ...
        function renderCalendarGrid() {
            ...
            daysToShow.forEach(day => {
                ...
                // Render slots
                HOURS.forEach(h => {
                    const slots = calendarAppointments.filter(a => {
                        const aDate = new Date(a.date_time || a.start_time);
        ...
        async function initReports() {
            ...
            try {
                ...
                // For now, empty tasks and contracts so it doesn't show fake money
                const tasks = [];
                const contracts = [];
        ...
        async function initSettings() {
            ...
            try {
                ...
                document.getElementById('set-ai-name').value = originalSettings.ai_config?.agent_name || "";
                document.getElementById('set-ai-goal').value = originalSettings.ai_config?.agent_goal || "";
                ...
                if (originalSettings.quick_replies && Array.isArray(originalSettings.quick_replies)) {
                    document.getElementById('set-quick-replies').value = originalSettings.quick_replies.join('\n');
                } else {
                    document.getElementById('set-quick-replies').value = "";
                }
        ...
        function switchSettingsTab(tab) {
            ...
            const btns = ['company', 'users', 'ai', 'messages', 'connections'];
            btns.forEach(b => {
            ...
            // Auto-load connection status when opening the Conexões tab
            if (tab === 'connections') {
        ...
        async function saveSettings() {
            ...
            const payload = {
                ...
                quick_replies: document.getElementById('set-quick-replies').value.split('\n').map(s => s.trim()).filter(s => s.length > 0)
            };
            ...
            } catch (e) { console.error("Save error", e); alert("Erro ao salvar configurações."); }
        ...
        function connectSettingsWhatsApp() {
            const tid = getMyTenantId();
            if (!tid) { alert('Erro: tenant não identificado.'); return; }
            connectTenantWhatsApp(tid);
        }

        function disconnectSettingsWhatsApp() {
            const tid = getMyTenantId();
            if (!tid) { alert('Erro: tenant não identificado.'); return; }
            disconnectTenantWhatsApp(tid);
        }

        function connectSettingsInstagram() {
            const tid = getMyTenantId();
            if (!tid) { alert('Erro: tenant não identificado.'); return; }
            connectTenantInstagram(tid);
        }
        ...
        async function loadConnectionsStatus() {
            try {
                ...
                if (res.ok) {
                    ...
                    const wppEl = document.getElementById('settings-wpp-status');
                    const instaEl = document.getElementById('settings-insta-status');

                    if (wppEl) {
                        const wpp = cfg.whatsapp_number;
                        if (wpp && wpp !== 'Aguardando Conexão' && wpp !== 'Pendente') {
                            wppEl.innerHTML = `<i class="fa-solid fa-circle text-[6px] mr-1 text-emerald-500"></i> <span class="text-emerald-400 font-medium">Conectado: ${wpp}</span>`;
                        } else {
                            wppEl.innerHTML = `<i class="fa-solid fa-circle text-[6px] mr-1 text-amber-500"></i> <span class="text-amber-400 font-medium">Desconectado</span>`;
                        }
                    }

                    if (instaEl) {
                        const integrations = cfg.integrations || {};
                        if (integrations.instagram_connected) {
                            instaEl.innerHTML = `<i class="fa-solid fa-circle text-[6px] mr-1 text-purple-500"></i> <span class="text-purple-400 font-medium">Instagram: Ativo</span>`;
                        } else {
                            instaEl.innerHTML = `<i class="fa-solid fa-circle text-[6px] mr-1 text-gray-500"></i> <span class="text-gray-400 font-medium">Não conectado</span>`;
                        }
                    }
                }
            } catch (e) { console.error('loadConnectionsStatus error:', e); }
        }
        ...
        async function inviteUser() {
            ...
            try {
                ...
                const data = await res.json();
                if (res.ok) {
                    alert("Convite enviado com sucesso para o e-mail: " + email);
                    document.getElementById('set-invite-email').value = "";
                    await loadSettingsUsers();
                } else {
                    alert("Erro ao enviar: " + (data.detail || "Falha no servidor"));
                }
            } catch (e) { 
                alert("Falha na rede ao tentar enviar convite.");
            } finally {
        ...
        function openLeadModalEncoded(encodedStr) {
            try {
                const lead = JSON.parse(decodeURIComponent(escape(atob(encodedStr))));
                openLeadModal(lead);
            } catch (e) {
